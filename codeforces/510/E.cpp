...
//------END-----------//
ll n,m,k,c[501],t,t1,i,j,res,pr[20001],used[501];
vector<int> odd,even,gt[501];
vector<vector<int>> cycles;
void dfs(int x)
{
	used[x]=1;
	cycles[k-1].push_back(x);
...
//    ofile;
    fio;
    for (i=2;i<=20000;i++) pr[i]=1;
    for (i=2;i*i<=20000;i++) if (pr[i])
    {
    	for (j=i*i;j<=20000;j+=i) pr[j]=0;
	}
    cin>>n;
    for (i=1;i<=n;i++)
    {
    	cin>>c[i];
    	if (c[i]%2==0) even.push_back(i); else odd.push_back(i);
	}
	dinic_graph gr(n+2);
	for (auto g: odd) gr.add_edge(0,g,2);
	for (auto g: even) gr.add_edge(g,n+1,2);
	for (auto g:odd) for (auto gg:even) if (pr[c[g]+c[gg]])
	{
//		cout<<g<<' '<<gg<<endl;
		gr.add_edge(g,gg,1);
	}
	pair<int,vector<edge>> pp=gr.detailed_calc_flow(0,n+1);
	if (pp.fi<n) {
		cout<<"Impossible"; return 0;
	}
	for (edge g: pp.second) 
		if ((g.u!=0)and(g.v!=n+1)and(c[g.u]%2==1)and(c[g.v]%2==0)and(g.cap==0))
	{
		gt[g.u].push_back(g.v);
		gt[g.v].push_back(g.u);
	}
